...
def extract_invoice_info(file_bytes: bytes, mime: str, filename: str) -> Dict:
    try:
        ...
        prompt = """Analyse cette facture et extrais les informations suivantes en JSON UNIQUEMENT:
{
...
Règles : Si une information est absente mets null. Pour le montant, chiffres uniquement sans symbole.
Réponds UNIQUEMENT avec le JSON, rien d'autre."""
        ...
        return {"fournisseur": None, "date": None, "commande": None,
...
def build_excel(records: List[Dict]) -> bytes:
    ...
    columns = [
        ("Fournisseur", "fournisseur", 25), ("Date", "date", 14),
        ("N° Commande", "commande", 18), ("Bon de Livraison", "bon_de_livraison", 20),
        ("N° Facture", "numero_facture", 20), ("Montant TTC", "montant_facture", 15),
...
with col_left:
    ...
    if uploaded_files:
        st.markdown(f'<div class="success-box">✅ {len(uploaded_files)} fichier(s) sélectionné(s)</div>',
                    unsafe_allow_html=True)
    ...
    if st.session_state.extracted_data:
        ...
        with col_dl:
            st.download_button(
                label="📥 Télécharger Excel", data=excel_bytes,
...
with col_right:
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            ...
            try:
                ...
                if "erreur" not in info:
                    st.markdown(f'<div class="success-box">✅ {uploaded_file.name} traité avec succès</div>',
                                unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="error-box">❌ {uploaded_file.name} : {info["erreur"]}</div>',
                                unsafe_allow_html=True)
            except Exception as e:
    ...
    if st.session_state.extracted_data:
        ...
        column_mapping = {
            'fournisseur': 'Fournisseur', 'date': 'Date', 'commande': 'Commande',
            'bon_de_livraison': 'Bon Livraison', 'numero_facture': 'N° Facture',
            'montant_facture': 'Montant (MAD)', 'fichier': 'Fichier'
        }
        df_display = df.rename(columns=column_mapping)
        display_cols = [c for c in ['Fournisseur', 'Date', 'N° Facture', 'Montant (MAD)', 'Fichier']
        ...
        valid_invoices = sum(1 for r in st.session_state.extracted_data
                             if parse_montant(r.get("montant_facture")) > 0)
        ...
        <div style="text-align:center;padding:60px 20px;background:#f8fafc;border-radius:8px;">
            ...
            <p style="font-size:1.2rem;color:#1e293b;">Aucune donnée extraite</p>
            <p style="color:#64748b;">Importez des factures à gauche et cliquez sur <strong>Extraire</strong></p>
        </div>
        """, unsafe_allow_html=True)
